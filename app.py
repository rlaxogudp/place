...
def result():
    ...
    while current_time < 24:
        ...
        if current_time >= 24:
            for category in used_indexes:
                used_indexes[category] = []
...
def make_block(hour, category, df):
    ...
    if not candidates.empty:
        ...
        available_i = [i for i in all_indexes if i not in used_indexes[category]]
        
        indexes = random.sample(available_i, k=min(3, len(available_i)))
        items = candidates.iloc[indexes]
        used_indexes[category] += indexes
